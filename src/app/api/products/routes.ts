...
export async function POST(req: Request) {
  try {
    ...
    // Check if the product already exists (based on name or sku, depending on requirements)
    const existingProduct = await prisma.product.findUnique({
      where: {
        sku, // Ensure the product SKU is unique (if SKU is a key identifier)
      },
    });
    ...
    const newProduct = await prisma.product.create({
      data: {
        ...
        sku,
        isActive,
        sellerId, // Use seller ID from request
      },
    });
